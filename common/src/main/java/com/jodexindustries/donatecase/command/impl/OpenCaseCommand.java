...
public class OpenCaseCommand extends SubCommand {

    private final DCAPI api;

    public OpenCaseCommand(DCAPI api) {
        super("opencase", api.getPlatform());
    ...
    public void execute(@NotNull CommandSender sender, @NotNull String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length >= 1) {
                ...
                if (data != null) {
                    CaseAnimation animation = api.getAnimationManager().getRegisteredAnimation(data.getAnimation());
                    ...
                    api.getCaseKeyManager().getKeysAsync(caseName, playerName).thenAccept((keys) -> {
                        if (keys >= 1) {
                            ...
                            api.getPlatform().getTools().msg(player, api.getConfig().getLang().getString("no-keys"));
                        }
                    });
